...
import { parentArticles } from '../data';
    ...
    <div className="relative">
            ...
            <div className="lg:col-span-5">
              <div className="glass rounded-[32px] p-8">
                ...
                <div className="mt-5 space-y-3">
                  {parentArticles.slice(0, 3).map((a) => (
                    <a key={a.title} href="#" className="block group">
                      <div className="text-xs text-ink-400">{a.tag} · {a.readTime}</div>
            ...
            return (
              ...
              >
                <Card className={`p-6 bg-gradient-to-br ${color} text-white hover:scale-[1.02] transition cursor-pointer`}>
                  ...
                  <div className="text-sm text-white/80 mt-1">{parentArticles.filter((a) => a.tag === t).length} articles</div>
      ...
      <Section>
        ...
        <Card className="overflow-hidden grid lg:grid-cols-2" hover={false}>
          <div className="aspect-[4/3] lg:aspect-auto overflow-hidden">
            <img src={parentArticles[0].image} alt="" className="h-full w-full object-cover" />
          </div>
          <div className="p-8 lg:p-14 flex flex-col justify-center">
            <div className="inline-flex items-center gap-2 rounded-full bg-blush-50 text-blush-700 px-3 py-1 text-xs font-semibold w-fit">{parentArticles[0].tag}</div>
            <h3 className="mt-5 font-serif text-3xl lg:text-4xl font-semibold text-ink-900 leading-tight">{parentArticles[0].title}</h3>
            <p className="mt-5 text-ink-500 leading-relaxed">{parentArticles[0].excerpt}</p>
            <div className="mt-8 flex items-center gap-4">
              <div className="flex items-center gap-2 text-sm text-ink-400"><Clock className="h-4 w-4" /> {parentArticles[0].readTime} read</div>
              <a href="#" className="rounded-full bg-ink-900 text-white px-5 py-2.5 text-sm font-semibold hover:bg-ink-800 transition flex items-center gap-2">
      ...
      <Section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {parentArticles.map((a, i) => {
            const color = topicColors[a.tag] || 'from-ink-400 to-ink-600';
            return (
              <motion.a
                href="#"
                key={a.title}
                ...
                <Card className="overflow-hidden h-full">
                  ...
                  <div className="p-7">
                    <div className="flex items-center gap-2 text-xs text-ink-400">
                      <Clock className="h-3.5 w-3.5" /> {a.readTime} read
                    </div>
                    ...
                    <p className="mt-3 text-sm text-ink-500 leading-relaxed line-clamp-3">{a.excerpt}</p>
